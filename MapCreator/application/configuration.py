...
class TabGeneral(tk.Frame):
    ...
    def __init__(self, master=None):
        ...
        entry_artist = tk.Entry(song_and_map_metadata_info_frame, textvariable=self.artist)
        entry_artist.grid(row=0, column=1, sticky="EW")
        self.romanised_artist = tk.StringVar()
        entry_romanised_artist = tk.Entry(song_and_map_metadata_info_frame, textvariable=self.romanised_artist)
        entry_romanised_artist.grid(row=1, column=1, sticky="EW")
        self.title = tk.StringVar()
        entry_title = tk.Entry(song_and_map_metadata_info_frame, textvariable=self.title)
        entry_title.grid(row=2, column=1, sticky="EW")
        self.romanised_title = tk.StringVar()
        entry_romanised_title = tk.Entry(song_and_map_metadata_info_frame, textvariable=self.romanised_title)
        entry_romanised_title.grid(row=3, column=1, sticky="EW")
        self.beatmap_creator = tk.StringVar()
        entry_beatmap_creator = tk.Entry(song_and_map_metadata_info_frame, textvariable=self.beatmap_creator)
        ...
        combobox_difficulty = ttk.Combobox(song_and_map_metadata_info_frame,
                                           values=["", "Easy", "Normal", "Hard", "Insane"], textvariable=self.difficulty)
        combobox_difficulty.grid(row=5, column=1, sticky="EW")
        self.source = tk.StringVar()
        entry_source = tk.Entry(song_and_map_metadata_info_frame, textvariable=self.source)
        entry_source.grid(row=6, column=1, sticky="EW")
        self.tags = tk.StringVar()
        entry_tags = tk.Entry(song_and_map_metadata_info_frame, textvariable=self.tags)
